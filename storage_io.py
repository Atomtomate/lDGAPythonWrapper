...
def run_conversion(path, out_formats):
    ...
    def read_and_convert(path, fn, header_names, output_formats):
        ...
        if len(output_formats):
            if "hdf5" in output_formats or "parquet" in output_formats:
                df = pd.read_csv(fn, sep=' ', skipinitialspace=True,
    ...
    def read_dir_and_convert(path, dn, header_names, output_formats):
        ...
        if "hdf5" in output_formats or "parquet" in output_formats:
            for fn in os.listdir(dn):
                dfi[fn[-3:]] = pd.read_csv(os.path.join(dn, fn), sep=' ',
    ...
    # ========================================================================
    fn = "chi_asympt"
    hn = ["omega", "Re_chi_up", "Im_chi_up", "Re_chi_do", "Im_chi_do",
          "Re_chi_ch", "Im_chi_ch"]
    print("storing " + fn)
    try:
    ...
    fn = "trip_omega"
    print("storing " + fn)
    hn = ["Re_Trip_up", "Im_Trip_up", "Re_Trip_do", "Im_Trip_do", "Re_Trip_pp",
          "Im_Trip_pp"]
    try:
        read_dir_and_convert(path, fn, hn, out_formats)
    except pandas.io.common.EmptyDataError:
